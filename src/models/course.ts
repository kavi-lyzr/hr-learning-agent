...
export interface ILesson {
  ...
  title: string;
  description?: string;
  contentType: 'video' | 'article' | 'video-article' | 'assessment'; // Can have both, or be assessment-only
  content: ILessonContent;
  duration: number; // Estimated duration in minutes
  order: number; // Position within module
  hasQuiz: boolean;
  quizData?: any; // Quiz questions JSON (to be generated by AI)
  isModuleAssessment?: boolean; // Flag to differentiate module assessments from lesson quizzes
  createdAt?: Date;
  updatedAt?: Date;
}
...
const LessonSchema = new Schema({
  ...
  contentType: {
    type: String,
    enum: ['video', 'article', 'video-article', 'assessment'],
    required: true
  },
  content: {
  ...
  isModuleAssessment: { type: Boolean, default: false } // Differentiates module assessments from lesson quizzes
}, {
  timestamps: true
});
